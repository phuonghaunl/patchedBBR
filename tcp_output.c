				      ...
				      u64 prior_wstamp)
{
	...
	//Ph added
	u64 tsedt, tsprior;
	tsedt = tp->tcp_wstamp_ns;
	tsprior = prior_wstamp;
	//
			   ...
			   int push_one, gfp_t gfp)
{
	...
	while ((skb = tcp_send_head(sk))) {
		...
		/*
                getnstimeofday(&now);
                num_pktsent = tp->segs_out;
                if(ntohs( sk->sk_dport)==5201){
                        printk("skdport= %lu %lu %d %d", (now.tv_sec *1000000 + now.tv_nsec /1000), num_pktsent, tso_segs, limit);
                }*/
                //
...
repair:
	...
	/*Ph added
	if(((u16) ntohs( sk->sk_dport))==5201){
		tcpdelta = (s64) hrtimer_get_softexpires_tv64(&tcp_sk(sk)->pacing_timer);
		printk("pktout = %u delta = %lld", tp->packets_out, tcpdelta);
	}
	*/
